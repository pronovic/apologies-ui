...
describe('Components/board/DownSlide.vue', () => {
    let wrapper

    beforeEach(() => {
        const div = document.createElement('div')
        document.body.appendChild(div)
        wrapper = shallowMount(DownSlide, {
            localVue,
            mocks,
            attachTo: div,
            propsData: props,
    ...
    test('component renders', async () => {
        ...
        expect(wrapper.findComponent({ ref: 'box' }).attributes().height).toBe(
            '30'
        )
        ...
        expect(wrapper.findComponent({ ref: 'circle' }).attributes().y).toBe(
            '22'
        ) // 30-8
